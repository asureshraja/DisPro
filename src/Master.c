...
void work(struct worker_args *args){
    ...
    args->response_body=malloc(512);
    if(strcmp(args->req->body,"connect")==0){
      strcpy(args->response_body,"YOU_ARE_CONNECTED");
    }else if(strncmp(args->req->body,"submit:",7)==0){
      strcpy(args->response_body,&args->req->body[7]);
    }else{
      strcpy(args->response_body,"I_AM_ALIVE");
    }
